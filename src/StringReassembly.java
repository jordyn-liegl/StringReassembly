...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        /*
         * subtract the substring with overlap in str1 and then concatenate str2
         */

        String prefix = str1.substring(0, str1.length() - overlap);
        String combo = prefix.concat(str2);
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * if input isn't a substring but there are elements that are a
         * substring of that input, remove those elements
         */

        //ensure that the set is non-empty
        if (strSet.size() > 0) {
            //try any random element of the set to see if there are any
            //substrings with the string
            String randElement = strSet.removeAny();

            //if the string isn't a substring of the random element, the string
            //and the new set will be called recursively to continue checking
            //until the set's size is 0
            if (!randElement.contains(str)) {
                addToSetAvoidingSubstrings(strSet, str);
            }
            //is the random element is a substring of the string, remove the
            //element and input the string
            if (!str.contains(randElement)) {
                strSet.add(randElement);
            }

        } else {
            //if the set is empty, the string can automatically be inputed
            //since there are no elements that it can be a substring of
            strSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //loop through each line until it reaches the end to continuously call
        // the addToSetAvoidingSubstrings to ensure that the lines are not
        //substrings of any other
        while (!input.atEOS()) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * loop through every character until a line separator is reached, then
         * add a new line
         */
        for (int i = 0; i < text.length(); i++) {
            if (text.charAt(i) == '~') {
                ...
                //if no line separator is reached just output the character
